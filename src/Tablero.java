public class Tablero {
    ...
    /**
     * Metodo que comprueba si en la coordenada recibida por parametro ya hay colocada una ficha
     *
     * @param c es una coordenada del tablero
     * @return true si la casilla está vacía
     */
    ...
    /**
     * Metodo que comprueba si hay linea en una fila del tablero
     *
     * @param fila es la fila del tablero a comprobar
     * @return true si todas las casillas de la fila recibia tienen la misma ficha
     */
    public boolean hayLineaHorizontal(int fila) {
        ...
        for (int i = 0; i < casillas[fila].length - 1; i++) {
            ...
            if (casillas[fila][i].estaVacia() || casillas[fila][i + 1].estaVacia())
                return false;
            if (casillas[fila][i].getFicha().equals(casillas[fila][i + 1].getFicha()))
                ++contador;
            //if (contador == casillas[fila].length - 1)
                //return true;
        }
    ...
    public boolean hayLineaVertical(int columna) {
        ...
        for (int i = 0; i < casillas[0].length - 1; i++) {
            ...
            if (casillas[i][columna].estaVacia() || casillas[i + 1][columna].estaVacia())
                return false;

            if (casillas[i][columna].getFicha().equals(casillas[i + 1][columna].getFicha()))
                ++contador;

            //if (contador == casillas[0].length - 1)
              //  return true;
        }
    ...
    public boolean hayDiagonalIzquierda() {
        ...
        for (int i = 0; i < casillas.length - 1; i++) {
            if (casillas[i][i].estaVacia() || casillas[i + 1][i + 1].estaVacia())
                return false;
            if (casillas[i][i].getFicha().equals(casillas[i + 1][i + 1].getFicha()))
                ++contador;
            //if (contador == casillas.length - 1)
              //  return true;
        }
    ...
    public boolean hayDiagonalDerecha() {
        ...
        for (int i = 0, j=fin; i <casillas.length-1 ; i++, j--) {
            ...
            if (casillas[i][j].estaVacia() || casillas[i+1][j-1].estaVacia())
                return false;
